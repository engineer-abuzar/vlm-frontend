...
export default function OtpVerificationPage() {
  ...
  const role = (sessionStorage.getItem("vlm_role") ?? "student") as Role;
  ...
  const handleVerify = () => {
    ...
    const phone = sessionStorage.getItem("vlm_phone") ?? "+919999999999";

    verifyMutation.mutate(
      { phone, otp: otpValue },
      {
        onSuccess: (data: AuthUser) => {
          const isNewUser = data.isNewUser ?? otpValue === "1234";
          navigate(resolvePostOtpPath(role, isNewUser), { replace: true });
        },
        onError: () => {
          // Dev fallback when API is unavailable
          const isNewUser = otpValue === "1234";
          localStorage.setItem("vlm_token", "dev-token");
          navigate(resolvePostOtpPath(role, isNewUser), { replace: true });
        },
      }
    );
  ...
  return (
    <div className="relative vlm-bg-navy flex min-h-svh w-full flex-col items-center bg-[#050505] px-6 overflow-hidden">
      ...
      <div className="relative z-10 mt-8 text-center space-y-4">
        <h2 className="text-3xl text-white tracking-tight">Verification Required</h2>
        <p className="text-white/50 text-[15px] leading-relaxed max-w-[280px] mx-auto">
          We've sent a 4-digit code to your mobile number.
        </p>
      </div>

      <Card className="relative z-10 mt-8 w-full max-w-[380px] border-white/10 bg-[#1a1a1a]/40 backdrop-blur-2xl rounded-[2.5rem] py-10 shadow-2xl">
        <CardContent className="flex flex-col items-center">
          <div className="text-center space-y-1 mb-8">
            <h3 className="text-white/90 text-base font-medium">Verify your Mobile</h3>
            <div className="flex items-center justify-center gap-2">
              <span className="text-white text-2xl font-bold tracking-tight">+91 99****3456</span>
              <div className="bg-green-500 rounded-full p-0.5">
                <CheckCircle2 size={16} className="text-[#050505] fill-current" />
              </div>
            </div>
            ...
            <InputOTPGroup className="gap-4">
              {[...Array(4)].map((_, i) => (
                <InputOTPSlot
                  ...
                  index={i}
                  className={cn(
                    "h-16 w-16 rounded-2xl border-none bg-[#333]/50 text-2xl font-bold text-white transition-all duration-300",
                    "data-[active=true]:ring-2 data-[active=true]:ring-indigo-500/50 data-[active=true]:shadow-[0_0_20px_rgba(99,102,241,0.4)]"
                  )}
                />
              ))}
            </InputOTPGroup>
          ...
          <div className="mt-6 flex items-center gap-2 bg-black/80 px-6 py-2.5 rounded-full border border-white/5 shadow-inner">
            ...
            <span className="text-white/90 text-[13px]">
              Time remaining: <span className="font-mono">00:{timer < 10 ? `0${timer}` : timer}</span>
            </span>
          </div>

          <div className="mt-8 text-center space-y-1">
            <p className="text-white/50 text-sm">Didn't receive code?</p>
            <Button
              variant="link"
